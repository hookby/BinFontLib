...
bool BinFontRuntime::loadFont(const char* path) {
    ...
    // 读取字体头
    auto readFunc = [](void* h, uint8_t* buf, size_t sz) -> size_t {
        return ((IBinFontPlatform*)h)->fileRead(h, buf, sz);
    };
    auto seekFunc = [](void* h, uint32_t pos) -> bool {
        return ((IBinFontPlatform*)h)->fileSeek(h, pos);
    };
    ...
    // 构建码点索引
    auto platformAlloc = [](void* ctx, size_t sz) -> void* {
        IBinFontPlatform* platform = (IBinFontPlatform*)ctx;
        return platform->memAlloc(sz);
    };
    
    _cpIndex = buildCpIndex(
        context, _header,
        platformRead, platformSeek,
        [](size_t sz) -> void* {
            // 这里需要访问platform，通过全局变量或其他方式
            // 暂时使用malloc
            return malloc(sz);
        }
    );
...
void BinFontRuntime::unload() {
    if (_cpIndex) {
        freeCpIndex(_cpIndex, [](void* p) { free(p); });
        _cpIndex = nullptr;
    }
    if (_path) {
        free(_path);
